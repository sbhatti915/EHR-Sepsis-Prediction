...
class ParseItemID(object):
    ...
    def __init__(self, ROOT=None):
        ...
        self.d_items = pd.read_csv(
            self.ROOT + "D_ITEMS.csv", usecols=["ITEMID", "LABEL"]
        )
        ...
        self.script_features = [
            ...
            "levofloxacin",
            "cipfloxacin(?!.*ophth)",
            "fluconazole(?! desensit)",
            "meropenem(?! )",
            "ceftriaxone(?! desensit)",
            "piperacillin",
            "ampicillin-sulbactam",
            "nafcillin",
            "oxacillin",
        ...
        self.script_patterns = [
            ".*" + feature + ".*" for feature in self.script_features
        ]

    def prescriptions_init(self):
        self.prescriptions = pd.read_csv(
            self.ROOT + "PRESCRIPTIONS.csv",
            usecols=["ROW_ID", "SUBJECT_ID", "HADM_ID", "DRUG", "STARTDATE", "ENDDATE"],
        )
    ...
    def extractor(self, feature_name, pattern):
        ...
        dictionary_value = (
            self.d_items["ITEMID"].where(condition).dropna().values.astype("int")
        )
...
class MimicParser(object):
    ...
    def __init__(self, ROOT=None):
        ...
        self.pid = ParseItemID(ROOT=self.ROOT)
    ...
    def reduce_total(self, filepath):
        ...
        pid = ParseItemID(ROOT=self.ROOT)
        ...
        # adding next specific dtypes to avoid "mixed dtype" error
        # read_dtypes={'VALUE':np.float64, 'RESULTSTATUS':np.float64, 'RESULTSTATUS':np.float64}
        for i, df_chunk in enumerate(
            pd.read_csv(filepath, iterator=True, chunksize=chunksize)
        ):
            function = lambda x, y: x.union(y)
            df = df_chunk[
                df_chunk["ITEMID"].isin(reduce(function, pid.dictionary.values()))
            ]
            df.dropna(inplace=True, axis=0, subset=columns)
            if i == 0:
                df.to_csv(
                    self.ROOT + "mapped_elements/CHARTEVENTS_reduced.csv",
                    index=False,
                    columns=columns,
                )
            else:
        ...
        if low_memory == False:
            ...
            for i in range(len(buckets)):
                for i, chunk in enumerate(
                    pd.read_csv(filename, iterator=True, chunksize=10000000)
                ):
                    print(buckets[i])
                    print(chunk["HADM_ID"].isin(buckets[i]))
                    sliced = chunk[chunk["HADM_ID"].astype("int").isin(buckets[i])]
                    sliced.to_csv(
                        self.ROOT + "mapped_elements/shard_{0}.csv".format(i),
                        index=False,
                    )
            ...
            for i in range(len(buckets)):
                with open(filename, "r") as chartevents:
                    ...
                    with open(
                        self.ROOT + "mapped_elements/shard_{0}.csv".format(i), "w"
                    ) as shard_writer:
    ...
    def create_day_blocks(self, file_name):
        ...
        df["CHARTDAY"] = (
            df["CHARTTIME"].astype("str").str.split(" ").apply(lambda x: x[0])
        )
        ...
        for i in list(df2.columns):
            df2[i][df2[i] > df2[i].quantile(0.95)] = df2[i].median()
            if i != "troponin":
                df2[i] = (
                    df2[i]
                    .where(df2[i] > df2[i].quantile(0.875))
                    .fillna(df2[i].median())
                )
    ...
    def add_admissions_columns(self, file_name):
        ...
        df_shard["HADM_ID"] = (
            df_shard["HADMID_DAY"].str.split("_").apply(lambda x: x[0])
        )
    ...
    def add_patient_columns(self, file_name):
        ...
        df = pd.read_csv(f"{self.ROOT}PATIENTS.csv")

        dob_dict = dict(zip(df["SUBJECT_ID"], df["DOB"]))
        gender_dict = dict(zip(df["SUBJECT_ID"], df["GENDER"]))
        df_shard = pd.read_csv(file_name)
        df_shard["SUBJECT_ID"] = df_shard["HADMID_DAY"].apply(
            lambda x: map_dict(x, self.hadm_dict)
        )
        df_shard["DOB"] = df_shard["SUBJECT_ID"].apply(lambda x: map_dict(x, dob_dict))

        df_shard["YOB"] = (
            df_shard["DOB"].str.split("-").apply(lambda x: x[0]).astype("int")
        )
        df_shard["ADMITYEAR"] = (
            df_shard["ADMITTIME"].str.split("-").apply(lambda x: x[0]).astype("int")
        )
    ...
    def clean_prescriptions(self, file_name):
        ...
        pid.prescriptions.to_csv(f"{self.ROOT}PRESCRIPTIONS_reduced.csv", index=False)

    def add_prescriptions(self, file_name):

        df_file = pd.read_csv(file_name)

        with open(f"{self.ROOT}PRESCRIPTIONS_reduced.csv", "r") as f:
            csvreader = csv.reader(f)
            with open(f"{self.ROOT}PRESCRIPTIONS_reduced_byday.csv", "w") as g:
        ...
        df = pd.read_csv(f"{self.ROOT}PRESCRIPTIONS_reduced_byday.csv")
        ...
        df_merged["HADM_ID"] = (
            df_merged["HADMID_DAY"].str.split("_").apply(lambda x: x[0])
        )
    ...
    def add_icd_infect(self, file_name):
        ...
        df_icd = pd.read_csv(f"{self.ROOT}PROCEDURES_ICD.csv")
        df_micro = pd.read_csv(f"{self.ROOT}MICROBIOLOGYEVENTS.csv")
    ...
    def add_notes(self, file_name):
        df = pd.read_csv(f"{self.ROOT}NOTEEVENTS.csv")
...
def do_all_parsing(ROOT, verbose=0):
    print("Doing data parsing. This should take a while")
    if ROOT[-1] != "/":
        ROOT += "/"
    ROOT += "mimic_database/"
    FOLDER = "mapped_elements/"
    FILE_STR = "CHARTEVENTS_reduced"
    mp = MimicParser(ROOT=ROOT)
    ...
    print("Total time to parse MIMIC-III dataset:", time.time() - start_time, "seconds")
